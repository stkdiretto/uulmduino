/*
TV-B-Gone für den uulmduino, Version 1.2ulm, 22. April 2015

Basierend auf TV-B-Gone for Arduino version 1.2, Oct 23 2010
von Ken Shirriff=
http://www.arcfn.com/2009/12/tv-b-gone-for-arduino.html

Dies ist auf die Arduino-IDE >1.0.5 und den uulmduino v.0.1 zugeschnitten
Abweichend von Ken Shirriffs Version wird Timer1 benutzt, um auf dem
Platinenlayout nicht Pin3 nutzen zu müssen.
 Der IR-Ausgang wird an Pin 10 angelegt (IRLED).
 Die eingebaute LED auf Pin 13 wird ebenfalls verwendet.
 Theoretisch kann ein Button als Trigger zwischen Pin 2 (TRIGGER) und Masse
    verwendet werden.
 Pin 5 (REGIONSWITCH) wird nicht benutzt.
...
/*
Dieses Projekt überträgt eine Reihe von TV-Ausschaltcodes, einen nach dem
 anderen, mit einer kurzen Pause zwischen jedem Code. Um direkt zu sehen,
 dass übertragen wird, blinkt auch die LED auf Pin13 bei jedem Code. Das ist
 alles, was TV-B-Gone tut. Der schwierige Teil war, alle Ausschaltcodes zu
 sammeln, und die Dubletten auszusortieren (wenn ein Code doppelt vorkommt,
 wird ein Fernseher erst aus- und dann bei der zweiten Übertragung wieder
 angeschaltet). Die Originale von Mitch Altman und Ken Shirriff haben die 
 üblichen Codes für Nordamerika (und auch Asien, obwohl dort vielfach PAL
 für die Übertragung verwendet wird) und Europa (passend auch in Australien,
 Neuseeland, im nahen Osten und anderen Teilen der Welt, wo PAL benutzt 
 wird) gesammelt.

 Diese Firmwareversion beinhaltet die jeweils über 100 meistgenutzten 
 Ausschaltcodes für Europa und Nordamerika.
 */
...
//      TCCR1A = _BV(COM1A0) | _BV(COM1B1) | _BV(WGM11);
